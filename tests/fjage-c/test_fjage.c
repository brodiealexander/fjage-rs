...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        if (gw == NULL)
            return error("Could not connect to fjage master container on serial port");
#endif
    }
    else
    {
        ...
        if (gw == NULL)
            return error("Could not connect to fjage master container on localhost:5081");
    }
    printf("Got here\n");
    fjage_aid_t myaid = fjage_get_agent_id(gw);
    printf("Got here\n");
    if (myaid != NULL)
        printf("get_agent_id> %s\n", myaid);
    ...
    int n = fjage_agents_for_service(gw, "org.arl.fjage.test.Services.PARAMETER_TEST", NULL, 0);

    // malloc n pointers where n = #max number of expected agents
    fjage_aid_t *agents = malloc(sizeof(void *) * n);
    int m = fjage_agents_for_service(gw, "org.arl.fjage.test.Services.PARAMETER_TEST", agents, n);
    test_assert("agents_for_service", n == 2 && m == 2 && agents[0] != NULL && !strcmp(agents[0], "gwtestbeta"));
    printf("NExt:%s\n", agents[0]);
    printf("NExt:%s\n", agents[1]);
    for (int i = 0; i < m; i++)
    {
    ...
    msg = fjage_receive(gw, NULL, NULL, 5000);
    test_assert("receive (+)", msg != NULL);
