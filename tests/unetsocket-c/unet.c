...
int unetsocket_send(unetsocket_t sock, uint8_t *data, int len, int to, int protocol)
{
  ...
  if (data != NULL)
    fjage_msg_add_byte_array(msg, "data", data, len);
  fjage_msg_add_int(msg, "to", to);
  fjage_msg_add_int(msg, "protocol", protocol);
  printf("Sending Message Out\n");
...
int unetsocket_send_reliable(unetsocket_t sock, uint8_t *data, int len, int to, int protocol)
{
  if (sock == NULL)
    return -1;
  if (to == 0)
    return -1; // broadcast with reliability is not allowed
  _unetsocket_t *usock = sock;
  fjage_msg_t msg;
  int rv;
  msg = fjage_msg_create("org.arl.unet.DatagramReq", FJAGE_REQUEST);
...
int unetsocket_send_request(unetsocket_t sock, fjage_msg_t req)
{
  ...
  if (protocol != DATA && (protocol < USER || protocol > MAX))
    return -1;
  printf("Checking protocol survived\n");
  recipient = fjage_msg_get_string(req, "recipient");
  if (recipient == NULL)
  {
    if (usock->provider == NULL)
      usock->provider = agent_for_service(usock, "org.arl.unet.Services.TRANSPORT");
    if (usock->provider == NULL)
      usock->provider = agent_for_service(usock, "org.arl.unet.Services.ROUTING");
    if (usock->provider == NULL)
      usock->provider = agent_for_service(usock, "org.arl.unet.Services.LINK");
    if (usock->provider == NULL)
      usock->provider = agent_for_service(usock, "org.arl.unet.Services.PHYSICAL");
    if (usock->provider == NULL)
      usock->provider = agent_for_service(usock, "org.arl.unet.Services.DATAGRAM");
    if (usock->provider == NULL)
      return -1;
...
fjage_msg_t unetsocket_receive(unetsocket_t sock)
{
  ...
  while (!usock->quit)
  {
    ...
    if (msg != NULL)
    {
      ...
      if ((rv == DATA || rv >= USER) && (usock->local_protocol < 0 || usock->local_protocol == rv))
        return msg;
    }
    if (usock->timeout == 0)
      return NULL;
